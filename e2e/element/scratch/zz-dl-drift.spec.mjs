...
test("EW-DLG: drift clicks with dragstart spy", async ({ page }) => {
  ...
  console.log(`>>> phase1 clean-click downloads=${firstCount}`);
  ...
  await page.evaluate(() => {
    ...
    document.addEventListener("dragstart", (e) => {
      const t = e.target;
      window.__drags.push({ tag: t.tagName, cls: String(t.className).slice(0, 80), href: t.href ? String(t.href).slice(0, 60) : null });
    }, true);
  });
  ...
  const humanClick = async (locator) => {
    ...
    await page.mouse.move(x + 5, y + 4, { steps: 6 }); // ~10px drift while pressed
    await page.waitForTimeout(80);
    await page.mouse.up();
  ...
  // A: drift-click the file tile (name anchor area)
  await humanClick(tile);
  console.log(`>>> A tile drift-click: downloads=${downloads.length} drags=${JSON.stringify(await page.evaluate(() => window.__drags))}`);

  // B: drift-click the hover action-bar download button
  await page.evaluate(() => { window.__drags = []; });
  await tile.hover();
  const dlBtn = page.getByRole("button", { name: /download/i }).first();
  if (await dlBtn.count()) await humanClick(dlBtn);
  console.log(`>>> B button drift-click: downloads=${downloads.length} drags=${JSON.stringify(await page.evaluate(() => window.__drags))}`);

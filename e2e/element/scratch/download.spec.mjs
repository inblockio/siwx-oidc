...
test("EW-DL1: encrypted file download fires a browser download", async ({ page }) => {
  ...
  for (const l of logs.filter((l) => /error|unsafe|404|SW:|worker|download|decrypt/i.test(l))) console.log(l);

  expect(downloads.length, "a browser download should have fired").toBeGreaterThan(0);
});

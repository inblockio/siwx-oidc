...
test("EW-DLL: labs flags vs download", async ({ page }) => {
  ...
  console.log(`>>> phase1 (clean account) downloads=${firstCount}`);

  // --- enable all labs flags via account data (per-account state like real users) ---
  const FLAGS = ["feature_ask_to_join","feature_bridge_state","feature_custom_themes","feature_disable_call_per_sender_encryption","feature_dynamic_room_predecessors","feature_element_call_video_rooms","feature_exclude_insecure_devices","feature_hidebold","feature_jump_to_date","feature_latex_maths","feature_location_share_live","feature_login_with_qr","feature_mjolnir","feature_msc3531_hide_messages_pending_moderation","feature_msc4095_url_preview_bundle","feature_msc4362_encrypted_state_events","feature_notification_settings2","feature_notifications","feature_render_reaction_images","feature_retention","feature_user_status","feature_video_rooms","feature_wysiwyg_composer"];
  await page.evaluate(async (flags) => {
    const cli = window.mxMatrixClientPeg.get();
    const cur = cli.getAccountData("im.vector.web.settings")?.getContent() ?? {};
    for (const f of flags) cur[f] = true;
    await cli.setAccountData("im.vector.web.settings", cur);
  }, FLAGS);
  await page.waitForTimeout(2000);
  ...
  const tile2 = page.getByText("dl-probe.pdf", { exact: false }).last();
  await tile2.waitFor({ timeout: 30_000 });
  await tile2.click();
  await page.waitForTimeout(6000);
  if (downloads.length === firstCount) {
    await tile2.hover();
    const b2 = page.getByRole("button", { name: /download/i }).first();
    if (await b2.count()) { await b2.click(); await page.waitForTimeout(6000); }
  }
  console.log(`>>> phase2 (ALL LABS ON) downloads=${JSON.stringify(downloads)}`);
  console.log("=== console lines ===");
  for (const l of logs.filter((l) => /error|eval|security|csp|download|decrypt/i.test(l))) console.log(l.slice(0, 260));
  console.log(`>>> VERDICT clean=${firstCount} withLabs=${downloads.length}`);
  expect(firstCount, "clean account").toBeGreaterThan(0);
});

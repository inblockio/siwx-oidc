...
test("EW-DLB: 5MB encrypted attachment downloads", async ({ page }) => {
  ...
  await page.evaluate(async (rid) => {
    const cli = window.mxMatrixClientPeg.get();
    const plaintext = new Uint8Array(5 * 1024 * 1024); for (let i = 0; i < plaintext.length; i += 65536) crypto.getRandomValues(plaintext.subarray(i, Math.min(i + 65536, plaintext.length)));
  ...
  console.log(`>>> phase1 (fresh login) downloads=${firstCount}`);

  // --- PHASE 2: restored session (what every real user actually runs) ---
  await page.reload({ waitUntil: "domcontentloaded" });
  await page.locator(".mx_MessageComposer").waitFor({ timeout: 60_000 });
  await page.waitForTimeout(6000); // let crypto/stores settle post-restore
  ...
  if (downloads.length === firstCount) {
    await tile2.hover();
    const dlBtn2 = page.getByRole("button", { name: /download/i }).first();
    if (await dlBtn2.count()) { await dlBtn2.click(); await page.waitForTimeout(6000); }
  }
  console.log(`>>> phase2 (restored) downloads=${JSON.stringify(downloads)}`);
  ...
  expect(firstCount, "phase1 fresh-login download").toBeGreaterThan(0);
  expect(downloads.length, "phase2 restored-session download").toBeGreaterThan(firstCount);
});

...
| **FINDING (P0-class, owner attention):** on reload, Element 1.12.20 + `force_verification` restores auth but NOT crypto: lands on "Confirm your digital identity" whose ONLY exits are "Use another device" or identity RESET — no recovery-key entry. Root-cause evidence (diag 2026-07-25): the wizard's backup/4S setup fully succeeded (POST room_keys/version 200, m.secret_storage.* + m.cross_signing.* account_data uploaded) AND the backup still exists at gate time (Element itself GETs room_keys/version → 200 on reload) — so this is an Element-build UX gap (no recovery-key self-verification offered) compounded by crypto verification state not surviving reload. NOT a siwx / lab-storage failure. Single-device users get a reload → verify-gate → forced-reset loop: lab REPRODUCTION of the prod "verify session loop / half-reset" forensics (2026-06-24 audits). EW-L1b pins this honestly and flags any behavior change | **Open** |

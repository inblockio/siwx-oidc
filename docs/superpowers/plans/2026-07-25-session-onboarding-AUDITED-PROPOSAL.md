...
| **FINDING 2 (reload → verify gate → reset loop) — ROOT-CAUSED 2026-07-25 (parallel-container matrix, vanilla 1.12.20 AND 1.12.24, with/without backup):** the gate does NOT reproduce on ANY vanilla Element — it is introduced by our vendored `force-first-device-recovery.patch` (matrix-server), whose extended `shouldForceVerification` (`!crossSigningReady \|\| !secretStorageReady`) also runs on session RESTORE, where secret-storage readiness transiently reads false before account_data re-hydrates. "Upgrade fixes it" = FALSE; NO upstream report warranted. **Fix belongs in the vendored patch** (gate only on the initial-login path, or re-evaluate after first sync); EW-L1b's chat-restore branch will flip green when fixed. Also noted: vanilla 1.12.2x has no forced first-device wizard at all (that is the patch too), and the patch targets 1.12.20 `MatrixChat.tsx` — it will likely not apply to 1.12.24 | **FIXED** (matrix-server `fix/finding2-restore-gate` v4: raw authed 4S probe in gate + forceReset, data-loss direction inverted; EW-L1b strict green 2026-07-26) |
| **FINDING 3 (NEW, P0 for any Element upgrade):** Element ≥ 1.12.24 (matrix-js-sdk v42, "OAuth2 → Matrix v1.18") CANNOT log in against current siwx-oidc: (1) v42 hard-requires `response_modes_supported` ⊇ {query, fragment} in auth metadata — siwx omits it → issuer rejected → legacy-SSO 404 dead-end; (2) v42 strictly requires `response_mode=fragment` delivery — siwx `sign_in` only emits query params → bounce loop. Fix = advertise + honor fragment response mode (together, never metadata-only). Guard WARNs on the missing field until then. See `docs/audits/2026-07-25-element-jssdk-v42-oauth-compat-finding.md` + evidence spec. **Do not bump Element past 1.12.23 (lab or prod) until fixed** | **FIXED** on `fix/finding3-fragment-response-mode` (fragment honored + advertised; vanilla 1.12.24 no-shim login green 2026-07-26; guard WARN→FAIL after prod deploy) |
...
| **Finding 1 guard:** `scripts/check-auth-metadata.sh` (fails on missing capability fields / non-public endpoint URLs; introspection exempt; legacy-404 WARN) + `/deploy-check` step 4; verified PASS on lab and prod, verified FAIL on a doctored fixture reproducing the bug shape | **Closed** |
| **`feat/passkey-offer-scoping` integrated:** cherry-picked `8e7fcfe` → `e1a8383` (clean; no conflicts vs honesty gate/3B). /account + /device passkey pickers now scoped via the opaque `siwx_user` cookie (degrade-open, `{"all":true}` escape). Validation on integrated tip: 115 unit / **26** mock browser (+2 scoping tests) / 17 Element, lab container rebuilt | **Done** |
| **P6 hermetic e2e-harness `full` vs branch tip** (siwx image retagged `localhost/siwx-oidc:local-grace` from the branch build; LiveKit UDP range moved to 20100–20200 below the ephemeral floor) | **PASS** — run `20260725-174923`: 16 pass / 0 fail / 1 known-flagged (msc4191 metadata re-export, documented Synapse 1.154 limitation) |

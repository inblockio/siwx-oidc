...
**Status:** ⚠️ **PARTIAL.** The verified-safe subset is **FIXED in `6e16b47`** — the login-path Expiration-Time is now enforced (covered by `tests/e2e_oauth_binding.rs`). The breaking remainder — server-issued single-use nonce binding on the device-approval + account-management paths (the full account-takeover fix) — is **still specced** in `2026-06-14-remediation-spec-criticals.md`, not shipped (it changes the CAIP-122 message shape and needs coordinated frontend + test edits).
...
**Status:** ⚠️ **PARTIAL.** The verified-safe subset is **FIXED in `6e16b47`** — open redirect at `/sign_in` re-validated, auth code↔client binding enforced at `/token`, and `plain` PKCE rejected (at `/authorize` and `/token`); covered by `tests/e2e_oauth_binding.rs`. The breaking remainder — making `redirect_uri` **mandatory** at `/token` and PKCE **mandatory** for public clients — is **still specced** in `2026-06-14-remediation-spec-criticals.md`, not shipped (changes the OAuth wire contract).
...
| H-g | **Token-type confusion** (access vs refresh interchangeable; no `mat_`/`mcr_`/type check on refresh & bearer paths). ~~+ REFRESH_TOKEN_TTL = 90 days vs 24h documented~~ | S2-4, S2-5 | **OWNER TRIAGE 2026-06-14:** the **90-day refresh TTL is INTENDED, not a bug** — the "vs 24h documented" was a documentation error, now corrected. The actionable remainder is **token-type tagging** (access vs refresh interchangeable): tag + check token type on every use. |
| H-h | **Unauthenticated, unbounded dynamic client registration** (`POST /register`, no auth/rate-limit, 30-day TTL) ⇒ Redis exhaustion + arbitrary redirect_uris feeding C2 | S4-3 | **OWNER TRIAGE 2026-06-14: ACCEPTED RISK — owner decision to leave DCR open; revisit on abuse.** **Not to be gated now.** (Original sketch retained for if/when revisited: gate DCR via initial access token or allow-list, and/or cap+rate-limit + constrain redirect_uris.) |
| H-i | **WebAuthn credentials & links stored with NO TTL**, creatable from a free anonymous session ⇒ permanent Redis growth + inflates the per-login `KEYS` scan into a login-time DoS | S4-4 | **OWNER TRIAGE 2026-06-14: accepted-for-now** (tied to the H-h DCR decision), but flagged as a **monitoring item** (Redis storage growth). (Original sketch: gate registration to C1's session; replace `KEYS` with an index; sweep unlinked anon credentials.) |
